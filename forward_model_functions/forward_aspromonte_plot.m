function [misfit] = forward_aspromonte_plot(sample,param)
...
if isfield(thermo_meas,'ahe')
    misfit_thermo=misfit_thermo+sum(log(2*pi/2)+log(thermo_meas.ahe_error)+0.5*((thermo_meas.ahe-modelled_data.ahe(1:6))./thermo_meas.ahe_error).^2);
end
if isfield(thermo_meas,'aft')
    misfit_thermo=misfit_thermo+sum(log(2*pi/2)+log(thermo_meas.aft_error)+0.5*((thermo_meas.aft-modelled_data.aft(7:9))./thermo_meas.aft_error).^2);
end
...
if isfield(thermo_meas,'aft_length')
    for i=1:length(thermo_meas.aft)
        if length(thermo_meas.aft_length(:,1))>length(thermo_meas.aft(i))
            misfit_thermo=misfit_thermo-sum(log(modelled_data.aft_length_pdf(i+6,(round((thermo_meas.aft_length(i,:)+0.1).*10)))))/201;
        else
            misfit_thermo=misfit_thermo+log(2*pi/2)+log(thermo_meas.aft_length_sd(i))+0.5*((thermo_meas.aft_length(i)-modelled_data.aft_MTL(i+6))./thermo_meas.aft_length_sd(i)).^2;
        end
    end
end
...
figure
subplot(1,3,1)
errorbar(thermo_meas.ahe,thermo_meas.ahe_error,'k.'); hold on
plot(thermo_meas.ahe,'ko','markerfacecolor',[0.5 0.5 0.5]);
plot(modelled_data.ahe(1:6),'ko','markerfacecolor',[0.1 0.1 0.9]);
title('AHe results')
xlabel('Sample')
ylabel('Age (Myr)')
...
plot(modelled_data.aft(7:9),'ko','markerfacecolor',[0.1 0.1 0.9]);
title('AFT results')
xlabel('Sample')
ylabel('Age (Myr)')
...
plot(modelled_data.aft_MTL(7:9),'ko','markerfacecolor',[0.1 0.1 0.9]);
title('AFT results')
xlabel('Sample')
